...
/// Gets the network id for the "BWNetwork" on the given `docker_host`.
/// Will create the network if it does not already exist.
pub fn get_bw_network_id(use_unix_socket: bool, docker_host: &str) -> ToolsetResult<String> {
    if let Ok(network) =
        dockurl::network::inspect_network("BWNetwork", docker_host, use_unix_socket, Simple::new())
    {
        Ok(network.id)
    } else {
