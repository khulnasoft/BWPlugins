...
pub fn create_container(
    ...
    match &config.network_mode {
        ...
        dockurl::network::NetworkMode::Host => {
            host_config.extra_host("bw-database", &config.database_host);
...
pub fn create_benchmarker_container(
    ...
    let mut options = Options::new();
    options.image("khulnasoft/bw.verifier");
    options.tty(true);
    options.attach_stderr(true);
    // The command_str we get back is an array of strings that make up the wrk
    // command; we want to replace `bw-server` with the IP address
    let mut command = vec![];
    for command_str in command_strs {
        command.push(command_str.replace("bw-server", &config.server_host));
    }
    ...
    match &config.network_mode {
        ...
        dockurl::network::NetworkMode::Host => {
            host_config.extra_host("bw-server", &config.server_host);
            host_config.network_mode(dockurl::network::NetworkMode::Host);
        }
    }
    let mut sysctls = HashMap::new();
...
/// Creates the container for the `BWVerifier`.
